part of '../mapvina_gl_platform_interface.dart';

/// Easing curve for [MapVinaMapController.easeCamera] animations.
///
...
/// to MapVina GL JS's `easeTo({easing})`, while MapVina Android only exposes
...
enum CameraAnimationInterpolation {
  ...
  ///
  /// Android: MapVina Android does not expose custom timing curves via
  ...
  ///
  /// Web: implemented exactly via MapVina GL JS `easeTo({easing})`.
  ///
  /// Android: MapVina Android does not expose custom timing curves via
  /// [easeCamera], so this falls back to the default ease-in/ease-out and
  /// is indistinguishable from [easeInOut] on Android.
...
/// The default instance of [MapVinaPlatform] to use.
typedef OnPlatformViewCreatedCallback = void Function(int);

abstract class MapVinaPlatform {
  static MapVinaMethodChannel? _instance;

  /// The default instance of [MapVinaPlatform] to use.
  ///
  /// Defaults to [MapVinaMethodChannel].
  ///
  /// Platform-specific plugins should set this with their own platform-specific
  /// class that extends [MapVinaPlatform] when they register themselves.
  static MapVinaPlatform Function() createInstance =
      () => _instance ?? MapVinaMethodChannel();

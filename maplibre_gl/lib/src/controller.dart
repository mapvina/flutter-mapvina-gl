...
/// Controller for a single [MapVinaMap] instance running on the host platform.
///
...
///
/// 1. *Simple way to add annotations*: Use the corresponding add* methods ([addCircle], [addLine], [addSymbol] and [addFill]) on the MapVinaMapController to add one annotation at a time to the map.
...
///
/// 2. *Advanced way to add annotations*: Modify the underlying MapVina Style of the map to add a new data source (e.g. with the [addSource] method or the more specific methods like [addGeoJsonSource])
...
/// For more information about MapVina Styles, see the documentation of [mapvina_gl] as well as the specification at [https://mapvina.com/mapvina-style-spec/].
///
/// A MapVinaMapController is also a [ChangeNotifier]. Subscribers (change listeners) are notified upon changes to any of
///
/// * the configuration options of the [MapVinaMap] widget
...
class MapVinaMapController extends ChangeNotifier {
  MapVinaMapController({
    required MapVinaPlatform mapvinaPlatform,
    ...
    this.onCameraIdle,
    this.onCameraMove,
  }) : _mapvinaPlatform = mapvinaPlatform {
    _cameraPosition = initialCameraPosition;

    _mapvinaPlatform.onFeatureTappedPlatform.add((payload) {
    ...
    _mapvinaPlatform.onFeatureDraggedPlatform.add((payload) {
      final id = payload["id"];
      final annotation = getAnnotationById(id);
    ...
    _mapvinaPlatform.onFeatureHoverPlatform.add((payload) {
      final id = payload["id"];
      final annotation = getAnnotationById(id);
    ...
    _mapvinaPlatform.onCameraMoveStartedPlatform.add((_) {
      _isCameraMoving = true;
      if (!isDisposed) notifyListeners();
    });

    _mapvinaPlatform.onCameraMovePlatform.add((cameraPosition) {
      _cameraPosition = cameraPosition;
      onCameraMove?.call(cameraPosition);
      if (!isDisposed) notifyListeners();
    });

    _mapvinaPlatform.onCameraIdlePlatform.add((cameraPosition) {
    ...
    _mapvinaPlatform.onMapStyleLoadedPlatform.add((_) async {
    ...
    _mapvinaPlatform.onMapClickPlatform.add((dict) {
      onMapClick?.call(dict['point'], dict['latLng']);
    });

    _mapvinaPlatform.onMapLongClickPlatform.add((dict) {
      onMapLongClick?.call(dict['point'], dict['latLng']);
    });

    _mapvinaPlatform.onMapMouseMovePlatform.add((payload) {
    ...
    _mapvinaPlatform.onCameraTrackingChangedPlatform.add((mode) {
      onCameraTrackingChanged?.call(mode);
    });

    _mapvinaPlatform.onCameraTrackingDismissedPlatform.add((_) {
      onCameraTrackingDismissed?.call();
    });

    _mapvinaPlatform.onMapIdlePlatform.add((_) {
      onMapIdle?.call();
    });
    _mapvinaPlatform.onUserLocationUpdatedPlatform.add((location) {
  ...
  /// Will be null, if [MapVinaMap.trackCameraPosition] is false.
  CameraPosition? get cameraPosition => _cameraPosition;
  CameraPosition? _cameraPosition;

  final MapVinaPlatform _mapvinaPlatform;
  ...
  Future<void> _updateMapOptions(Map<String, dynamic> optionsUpdate) async {
    _cameraPosition = await _mapvinaPlatform.updateMapOptions(optionsUpdate);
    if (!isDisposed) notifyListeners();
  }
  ...
  Future<bool?> animateCamera(
    ...
    return _mapvinaPlatform.animateCamera(cameraUpdate, duration: duration);
  }
  ...
  Future<bool?> moveCamera(CameraUpdate cameraUpdate) async {
    return _mapvinaPlatform.moveCamera(cameraUpdate);
  }
  ...
  /// id of the feature. This is useful because by default mapvina-gl-js does not
  /// support string ids
  ///
  /// The returned [Future] completes after the change has been made on the
  /// platform side.
  Future<void> addGeoJsonSource(
    ...
    await _mapvinaPlatform.addGeoJsonSource(
      sourceId,
      geojson,
      promoteId: promoteId,
    );
  ...
  Future<void> setGeoJsonSource(
    ...
    await _mapvinaPlatform.setGeoJsonSource(sourceId, geojson);
  }
  ...
  Future<void> setGeoJsonFeature(
    ...
    await _mapvinaPlatform.setFeatureForGeoJsonSource(
      sourceId,
      geojsonFeature,
    );
  }
  ...
  Future<void> setFeatureState(
    ...
    await _mapvinaPlatform.setFeatureState(
      sourceId,
      featureId,
      state,
  ...
  Future<void> removeFeatureState(
    ...
    await _mapvinaPlatform.removeFeatureState(
      sourceId,
      featureId: featureId,
      stateKey: stateKey,
      sourceLayer: sourceLayer,
  ...
  Future<Map<String, dynamic>?> getFeatureState(
    ...
    final result = await _mapvinaPlatform.getFeatureState(
      sourceId,
      featureId,
      sourceLayer: sourceLayer,
    );
  ...
  ///
  /// [expressions]: https://mapvina.com/mapvina-style-spec/expressions/
  Future<void> addSymbolLayer(
    ...
    await _mapvinaPlatform.addSymbolLayer(
      sourceId,
      layerId,
      properties.toJson(),
  ...
  Future<void> addLineLayer(
    ...
    await _mapvinaPlatform.addLineLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> setLayerProperties(
    ...
    await _mapvinaPlatform.setLayerProperties(
      layerId,
  ...
  Future<void> addFillLayer(
    ...
    await _mapvinaPlatform.addFillLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> addFillExtrusionLayer(
    ...
    await _mapvinaPlatform.addFillExtrusionLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> addCircleLayer(
    ...
    await _mapvinaPlatform.addCircleLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> addRasterLayer(
    ...
    await _mapvinaPlatform.addRasterLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> addHillshadeLayer(
    ...
    await _mapvinaPlatform.addHillshadeLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> addHeatmapLayer(
    ...
    await _mapvinaPlatform.addHeatmapLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> updateMyLocationTrackingMode(
    ...
    return _mapvinaPlatform.updateMyLocationTrackingMode(
  ...
  Future<void> matchMapLanguageWithDeviceDefault() async {
    return _mapvinaPlatform.matchMapLanguageWithDeviceDefault();
  }
  ...
  Future<void> updateContentInsets(
    ...
    return _mapvinaPlatform.updateContentInsets(insets, animated);
  }
  ...
  /// In that case, you need to adapt your MapVina style accordingly yourself to use labels in your preferred language.
  ///
  ...
  Future<void> setMapLanguage(String language) async {
    return _mapvinaPlatform.setMapLanguage(language);
  }
  ...
  Future<void> setTelemetryEnabled(bool enabled) async {
    return _mapvinaPlatform.setTelemetryEnabled(enabled);
  }
  ...
  Future<bool> getTelemetryEnabled() async {
    return _mapvinaPlatform.getTelemetryEnabled();
  }
  ...
  Future<void> setMaximumFps(int fps) async {
    return _mapvinaPlatform.setMaximumFps(fps);
  }
  ...
  Future<void> forceOnlineMode() async {
    return _mapvinaPlatform.forceOnlineMode();
  }
  ...
  Future<bool> easeCamera(
    ...
    return _mapvinaPlatform.easeCamera(
      cameraUpdate,
      duration: duration,
  ...
  Future<CameraPosition?> queryCameraPosition() async {
    return _mapvinaPlatform.queryCameraPosition();
  }
  ...
  Future<bool> editGeoJsonSource(String id, String data) async {
    return _mapvinaPlatform.editGeoJsonSource(id, data);
  }
  ...
  Future<bool> editGeoJsonUrl(String id, String url) async {
    return _mapvinaPlatform.editGeoJsonUrl(id, url);
  }
  ...
  Future<bool> setLayerFilter(String layerId, String filter) async {
    return _mapvinaPlatform.setLayerFilter(layerId, filter);
  }
  ...
  Future<String?> getStyle() async {
    return _mapvinaPlatform.getStyle();
  }
  ...
  Future<void> setCustomHeaders(
    ...
    return _mapvinaPlatform.setCustomHeaders(headers, filter);
  }
  ...
  Future<Map<String, String>> getCustomHeaders() async {
    return _mapvinaPlatform.getCustomHeaders();
  }
  ...
  Future<List> queryRenderedFeatures(
    ...
    return _mapvinaPlatform.queryRenderedFeatures(point, layerIds, filter);
  }
  ...
  Future<List> queryRenderedFeaturesInRect(
    ...
    return _mapvinaPlatform.queryRenderedFeaturesInRect(
      rect,
      layerIds,
      filter,
    );
  ...
  Future<List> querySourceFeatures(
    ...
    return _mapvinaPlatform.querySourceFeatures(
      sourceId,
      sourceLayerId,
      filter,
    );
  ...
  Future invalidateAmbientCache() async {
    return _mapvinaPlatform.invalidateAmbientCache();
  }

  Future clearAmbientCache() async {
    return _mapvinaPlatform.clearAmbientCache();
  }
  ...
  Future<LatLng?> requestMyLocationLatLng() async {
    return _mapvinaPlatform.requestMyLocationLatLng();
  }

  /// This method returns the boundaries of the region currently displayed in the map.
  Future<LatLngBounds> getVisibleRegion() async {
    return _mapvinaPlatform.getVisibleRegion();
  }
  ...
  Future<void> addImage(String name, Uint8List bytes, [bool sdf = false]) {
    return _mapvinaPlatform.addImage(name, bytes, sdf);
  }
  ...
  Future<void> addImageSource(
    ...
    return _mapvinaPlatform.addImageSource(imageSourceId, bytes, coordinates);
  }
  ...
  Future<void> updateImageSource(
    ...
    return _mapvinaPlatform.updateImageSource(
      imageSourceId,
      bytes,
      coordinates,
    );
  ...
  Future<void> removeSource(String sourceId) {
    return _mapvinaPlatform.removeSource(sourceId);
  }
  ...
  Future<void> addImageLayer(
    ...
    return _mapvinaPlatform.addLayer(layerId, imageSourceId, minzoom, maxzoom);
  }
  ...
  Future<void> addImageLayerBelow(
    ...
    return _mapvinaPlatform.addLayerBelow(
      layerId,
      sourceId,
      imageSourceId,
      minzoom,
  ...
  /// Removes a MapVina style layer
  Future<void> removeLayer(String layerId) {
    return _mapvinaPlatform.removeLayer(layerId);
  }

  Future<void> setFilter(String layerId, dynamic filter) {
    return _mapvinaPlatform.setFilter(layerId, filter);
  }

  Future<dynamic> getFilter(String layerId) {
    return _mapvinaPlatform.getFilter(layerId);
  }
  ...
  Future<Point> toScreenLocation(LatLng latLng) async {
    return _mapvinaPlatform.toScreenLocation(latLng);
  }

  Future<List<Point>> toScreenLocationBatch(Iterable<LatLng> latLngs) async {
    return _mapvinaPlatform.toScreenLocationBatch(latLngs);
  }
  ...
  Future<LatLng> toLatLng(Point screenLocation) async {
    return _mapvinaPlatform.toLatLng(screenLocation);
  }

  /// Returns the distance spanned by one pixel at the specified [latitude] and current zoom level.
  /// The distance between pixels decreases as the latitude approaches the poles. This relationship parallels the relationship between longitudinal coordinates at different latitudes.
  Future<double> getMetersPerPixelAtLatitude(double latitude) async {
    return _mapvinaPlatform.getMetersPerPixelAtLatitude(latitude);
  }

  /// Add a new source to the map
  Future<void> addSource(String sourceid, SourceProperties properties) async {
    return _mapvinaPlatform.addSource(sourceid, properties);
  }
  ...
  Future setCameraBounds({
    ...
    return _mapvinaPlatform.setCameraBounds(
      west: west,
      north: north,
      south: south,
      east: east,
  ...
  Future<void> setLayerVisibility(String layerId, bool visible) async {
    return _mapvinaPlatform.setLayerVisibility(layerId, visible);
  }

  Future<List> getLayerIds() {
    return _mapvinaPlatform.getLayerIds();
  }
  ...
  Future<List<String>> getSourceIds() async {
    return (await _mapvinaPlatform.getSourceIds())
        .whereType<String>()
        .toList();
  }
  ...
  Future<bool?> getLayerVisibility(String layerId) {
    return _mapvinaPlatform.getLayerVisibility(layerId);
  }
  ...
  Future<Size> setWebMapToCustomSize(Size size) {
    return _mapvinaPlatform.setWebMapToCustomSize(size);
  }

  /// Waits until the map is idle after camera movement.
  Future<void> waitUntilMapIsIdleAfterMovement() {
    return _mapvinaPlatform.waitUntilMapIsIdleAfterMovement();
  }
  ...
  Future<void> waitUntilMapTilesAreLoaded() {
    return _mapvinaPlatform.waitUntilMapTilesAreLoaded();
  }
  ...
  Future<Uint8List> takeSnapshot({int? width, int? height}) {
    return _mapvinaPlatform.takeSnapshot(width: width, height: height);
  }

  /// Method to set style string
  /// A MapVina GL style document defining the map's appearance.
  /// The style document specification is at [https://mapvina.com/mapvina-style-spec].
  /// A short introduction can be found in the documentation of the [mapvina_gl] library.
  ...
  Future<void> setStyle(String styleString) async {
    return _mapvinaPlatform.setStyle(styleString);
  }

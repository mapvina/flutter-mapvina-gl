 ...
 * the map. A Texture drawn using MapVinaMap bitmap snapshots can then be shown instead of the
 * overlay.
 */
public class MapVinaMapsPlugin implements FlutterPlugin, ActivityAware {

  static FlutterAssets flutterAssets;
  private Lifecycle lifecycle;

  public MapVinaMapsPlugin() {
    // no-op
  }
  ...
  public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
    ...
    // Reduce MapVina SDK logging verbosity to prevent spam from HTTP requests
    ...
    MethodChannel methodChannel =
        new MethodChannel(binding.getBinaryMessenger(), "plugins.flutter.io/mapvina_gl");
    methodChannel.setMethodCallHandler(new GlobalMethodHandler(binding));

    binding
        .getPlatformViewRegistry()
        .registerViewFactory(
            "plugins.flutter.io/mapvina_gl",
            new MapVinaMapFactory(

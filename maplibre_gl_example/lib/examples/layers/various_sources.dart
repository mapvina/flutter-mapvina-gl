...
class StyleInfo {
  ...
  final Future<void> Function(MapVinaMapController) addDetails;
...
class FullMapState extends State<FullMap> {
  ...
  static Future<void> addRaster(MapVinaMapController controller) async {
  ...
  static Future<void> addGeojsonCluster(
    ...
    await controller.addSource(
      ...
      const GeojsonSourceProperties(
        data:
            'https://mapvina.com/mapvina-gl-js/docs/assets/earthquakes.geojson',
        cluster: true,
        clusterMaxZoom: 14, // Max zoom to cluster points on
        clusterRadius:
            50, // Radius of each cluster when clustering points (defaults to 50)
        attribution:
            '<a href="https://mapvina.com">Earthquake data © MapVina</a>',
      ),
    );
    await controller.addLayer(
      "earthquakes",
  ...
  static Future<void> addVector(MapVinaMapController controller) async {
    await controller.addSource(
      "openmaptiles",
      const VectorSourceProperties(
        ...
        maxzoom: 14,
        attribution:
            '<a href="https://mapvina.com">© MapVina contributors</a',
      ),
    );
  ...
  static Future<void> addImage(MapVinaMapController controller) async {
    await controller.addSource(
      "radar",
      const ImageSourceProperties(
        url: "https://mapvina.com/mapvina-gl-js/docs/assets/radar.gif",
        coordinates: [
          [-80.425, 46.437],
          [-71.516, 46.437],
          [-71.516, 37.936],
  ...
  static Future<void> addVideo(MapVinaMapController controller) async {
    await controller.addSource(
      "video",
  ...
  static Future<void> addHeatMap(MapVinaMapController controller) async {
    await controller.addSource(
  ...
  static Future<void> addCountries(MapVinaMapController controller) async {
  ...
  static Future<void> addDemHillshade(MapVinaMapController controller) async {
  ...
  final _stylesAndLoaders = [
    const StyleInfo(
      name: "Vector",
      baseStyle: MapVinaStyles.demo,
      addDetails: addVector,
      position: CameraPosition(target: LatLng(33.3832, -118.4333), zoom: 6),
    ),
    const StyleInfo(
      name: "Countries GeoJSON",
      // Using the raw github file version of MapVinaStyles.DEMO here, because we need to

...
class MapVinaMapController: NSObject, FlutterPlatformView, MLNMapViewDelegate, MapVinaMapOptionsSink,
    UIGestureRecognizerDelegate
{
    ...
    private static func createMapView(
        ...
        NSLog(
            """
            Warning: MapVinaMapController - Initializing map view with \
            default style. This capability will be removed in a future release.
            """
        )
        // https://github.com/mapvina/mapvina-native/issues/709
    ...
    init(
        ...
        channel = FlutterMethodChannel(
            name: "plugins.flutter.io/mapvina_gl_\(viewId)",
        ...
        if let args = args as? [String: Any] {
            ...
            Convert.interpretMapVinaMapOptions(options: args["options"], delegate: self)
    ...
    func onMethodCall(methodCall: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "map#update":
            ...
            Convert.interpretMapVinaMapOptions(options: arguments["options"], delegate: self)
        ...
        case "map#forceOnlineMode":
            ...
            // In MapVina GL iOS, this is typically handled by the style and data sources
            result(nil)
        case "camera#ease":
        ...
        case "map#setCustomHeaders":
            ...
            MapVinaCustomHeaders.setCustomHeaders(headers, filter: filter)
            result(nil)
        case "map#getCustomHeaders":
            result(MapVinaCustomHeaders.getCustomHeaders())
    ...
    private func firstFeatureOnLayers(at: CGPoint) -> (feature: MLNFeature?, layerId: String?) {
        guard let style = mapView.style else { 
            NSLog("MapVinaMapController - Map style is nil")
            return (nil, nil) 
        }
        
        guard styleIsReady else { 
            NSLog("MapVinaMapController - Map style is not ready yet")
            return (nil, nil) 
        }
    ...
    func setMinMaxZoomPreference(min: Double?, max: Double?) {
        // Use MapVina defaults (0 for min, 22 for max) when unbounded (nil)
        let minZoom = min ?? 0.0
